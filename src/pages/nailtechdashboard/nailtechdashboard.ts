...
export class NailtechdashboardPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public http: Http, public modalCtrl: ModalController) {
    ...
    if(this.data.isVendor === 1){
      ...
      this.http.post('api/appointment/getappointment', ({"userId": this.data.id}))
       ...
       this.http.post('api/review/getreviews', ({"userId": this.data.id}))
        .subscribe(reviews => {
      ...
      this.bookFlag = true
      this.http.post('api/appointment/clientappointments', ({"clientId": this.data.id}))
        .subscribe(appointment => {
        let result = appointment.json()
