...
export class NailtechdashboardPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public http: Http, public modalCtrl: ModalController) {
    ...
    if(this.data.isVendor === 1){
      ...
      this.http.post('https://ec2-54-201-208-89.us-west-2.compute.amazonaws.com:1443/api/appointment/getappointment', ({"userId": this.data.id}))
       ...
       this.http.post('https://ec2-54-201-208-89.us-west-2.compute.amazonaws.com:1443/api/review/getreviews', ({"userId": this.data.id}))
        .subscribe(reviews => {
        ...
        for(let k = 0; k < result.length; k++){
         ...
         this.monthlyAvgs[11] = this.monthlyAvgs[11]/(countDec*100)
         this.monthlyAvgs[10] = this.monthlyAvgs[10]/(countNov*100)
         this.monthlyAvgs[9] = this.monthlyAvgs[9]/(countOct*100)
         this.monthlyAvgs[8] = this.monthlyAvgs[8]/(countSep*100)
         this.monthlyAvgs[7] = this.monthlyAvgs[7]/(countAug*100)
         this.monthlyAvgs[6] = this.monthlyAvgs[6]/(countJul*100)
         this.monthlyAvgs[5] = this.monthlyAvgs[5]/(countJun*100)
         this.monthlyAvgs[4] = this.monthlyAvgs[4]/(countMay*100)
         this.monthlyAvgs[3] = this.monthlyAvgs[3]/(countApr*100)
         this.monthlyAvgs[2] = this.monthlyAvgs[2]/(countMar*100)
         this.monthlyAvgs[1] = this.monthlyAvgs[1]/(countFeb*100)
         this.monthlyAvgs[0] = this.monthlyAvgs[0]/(countJan*100)
      ...
      this.bookFlag = true
      this.http.post('https://ec2-54-201-208-89.us-west-2.compute.amazonaws.com:1443/api/appointment/clientappointments', ({"clientId": this.data.id}))
        .subscribe(appointment => {
        let result = appointment.json()
